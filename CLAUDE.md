...
**M0 is complete.** The scaffold, CI workflows, and spikes SP1–SP4 are in place, and all four spikes pass on WSL2 + Docker Desktop (reports in [docs/spikes/](docs/spikes/), demo in [docs/demos/M0.md](docs/demos/M0.md)). ADR-0003, ADR-0004, and ADR-0005 are validated; no fallback was needed.

Still outstanding from M0: the spikes have **not** run on native Docker yet (the `Spikes` workflow covers it), and CI has not gone green on GitHub because no pull request exists yet.

The next step is **M1** in [docs/ROADMAP.md](docs/ROADMAP.md): `nephos up`/`down`, the SQLite store and reconcile framework, the OpenAPI skeleton, and two instances in different subnets that can ping each other. Don't write code outside the current milestone's scope.

**Carry these M0 findings into M1 and M2** — each was a plausible-but-wrong result first, and they are written up in the spike reports:

- The appliance entrypoint must delegate the cgroup v2 controllers to a leaf cgroup, and **fail closed** if `memory`, `pids`, or `cpu` cannot be delegated.
- Instances need `--cap-add NET_ADMIN`; Podman's default set omits it.
- AMIs need `APT::Sandbox::User "root"`, or every `apt` command fails under `userns=auto`.
- sshd host keys are generated by an `ExecStartPre` that **resets the list first** (drop-ins append), and socket activation stays off so a listening port cannot mean a dead sshd.
- The IMDS must serve **dated API versions** (`/2009-04-04/…`), not only `/latest`, or cloud-init falls back to `DataSourceNone` while still reporting success.
- `ds-identify` disables cloud-init in containers unless its policy is forced, and cloud-init merges config lists by **appending**, so removing a module means editing `/etc/cloud/cloud.cfg` itself.

The `spikes/` tree is throwaway and is a **separate Go module**, so it adds no dependencies to the main one and `go test ./...` at the root does not see it.
